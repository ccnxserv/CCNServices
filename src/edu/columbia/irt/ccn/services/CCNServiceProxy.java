...
/**
 * CCNServiceProxy is a file system proxy that makes files on the local system
 ...
 * following contents: /foo/ bar.txt baz/ box.txt and you call CCNServiceProxy
 * /foo ccnx:/testprefix
 * 
...
public class CCNServiceProxy implements CCNFilterListener {
	...
	public static void usage() {
		System.err
				.println("usage: CCNServiceProxy <file path to serve> [<ccn prefix URI> default: ccn:/]");
	}
	...
	public CCNServiceProxy(String filePrefix, String ccnxURI)
		...
		_responseName = KeyProfile.keyName(null, _handle.keyManager()
				.getDefaultKeyID());
	...
	protected String ccnNametoFile(ContentName name) {
	...
	protected File ccnNameToFilePath(Interest interest, ContentName name) {
		...
		String[] tokens = fn.split("\\%2B");
		if (tokens.length > 1) {
			addService(interest, _rootDirectory.toString(), tokens);
		}
	...
	protected boolean writeFile(Interest outstandingInterest)
		...
		File check = new File(_rootDirectory, "/"
				+ ccnNametoFile(outstandingInterest.name()));
		if (!check.exists()) {
			return (false);
		}
		if (processing == true) {
		...
		ContentName versionedName = VersioningProfile.addVersion(
				new ContentName(_prefix, outstandingInterest.name()
						.postfix(_prefix).components()), modificationTime);
		...
		/* Done with file transfer. Flag it */
		fileTransfer = false;
		return true;
	}
			...
			if (interest.exclude().size() > 1) {
				...
				if ((null != el) && (el instanceof ExcludeComponent)) {
					Log.info(
							"previous version: {0}",
							VersioningProfile
									.getVersionComponentAsTimestamp(((ExcludeComponent) el)
											.getBytes()));
				}
			}
		}
